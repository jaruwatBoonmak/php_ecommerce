<?php
  ...
  if (isset($_SESSION['superuser_email'])) {
    $userEmail = $_SESSION['superuser_email'];
        ...
        <div class="headBar">
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
                ...
                <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="log.php">ประวัติ</a>
                    </li>
                    </ul>
                    <form class="d-flex">
                        <?php
                        if (isset($_SESSION['superuser_email'])) {
                            ?>
                            <a class="nav-link active" aria-current="page" href="#"><?php echo $_SESSION['superuser_email'];?></a>
        ...
        <div class="tableTotal" style="padding:20px;">
            ...
            <tbody>
                <?php
                    ...
                    {
                        $sql = "SELECT * FROM user WHERE user_email='$row[cus_id]'";
                        $result = mysqli_query($link, $sql);
                        $row2 = mysqli_fetch_assoc($result);
                        if($row['order_status'] == 'รอพิจารณา'){
                            ?>
                                
                                <tr>
                                    <td><?php echo $row['order_detail']; ?></td>
                                    <td><?php echo $row['cus_id'];?></td>
                                    <td><?php echo $row2['user_add'];?></td>
                                    <td>
                                        <form method="post" action="../function/submitOder.php?order_id=<?php echo $row['order_id']; ?>">
                                            <input type="submit" name="yes" class="btn btn-primary btn-sm" style="margin: 10px;" value="ตกลง"></input>
                                            <input type="submit" name="no" class="btn btn-primary btn-sm" style="margin: 10px;" value="ยกเลิก"></input>
                                        </form>
                                    </td>
    ...
    <?php
    include '../function/submitOder.php';
}else{
